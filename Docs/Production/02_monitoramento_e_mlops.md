...
* **Detecção:**
  * **Sinal primário — cobertura de recuperação.** Para cada consulta registramos a **similaridade de cosseno máxima** entre a alegação canônica e os chunks recuperados. Consultas com $\text{sim}_{max} < 0{,}85$ são marcadas como `low_coverage`. Se a média móvel de 7 dias dessa taxa ultrapassar **15%** das requisições, dispara alerta.

    > **O limiar depende do modelo de embedding e precisa ser medido, não arbitrado.** O `multilingual-e5-base` produz similaridades de cosseno com linha de base alta: medido contra a base de 1.480 chunks (2026-09-09), uma pergunta sobre astrofísica pontuou **0,825** e uma sobre configuração de servidor, **0,807** — ambas totalmente fora do domínio. Consultas legitimamente no tema ficaram entre **0,86 e 0,88**. A faixa útil é estreita: 0,78 (ruído) a 0,88 (no tema). Um limiar de 0,75, como previsto originalmente, nunca dispararia. Ao trocar de modelo de embedding, **refaça esta medição** — a escala de similaridade muda junto.
